...
parser.add_argument('--train_dataset', default='train_2,train_3', type=str)
parser.add_argument('--test_dataset', default='test_2,train_1', type=str)
...
def main(args):
	...
	list_f_train = args.train_dataset.split(',')
	list_f_test = args.test_dataset.split(',')
	file_format = '.jpeg'
	
	# process train dataset
	for f_train in list_f_train:
		# read files into label and frame lists
		with open('../data/labels/' + f_train + '_' + args.data_type + '_label.csv') as f:
			frames_labels = [(line.strip().split(',')[0], line.strip().split(',')[1]) for line in f]
		file_dir = '../data/' + f_train + '/' + args.data_type + '/jpg/'
		for fr_lb in frames_labels:
			fr, lb  = fr_lb
			train_filenames.append(file_dir + fr + file_format)
			train_labels.append(int(lb) - 1)
		
	# process test dataset
	for f_test in list_f_test:
		# read files into label and frame lists
		with open('../data/labels/' + f_test + '_' + args.data_type + '_label.csv') as f:
			frames_labels = [(line.strip().split(',')[0], line.strip().split(',')[1]) for line in f]
		file_dir = '../data/' + f_test + '/' + args.data_type + '/jpg/'
		for fr_lb in frames_labels:
			fr, lb  = fr_lb
			val_filenames.append(file_dir + fr + file_format)
