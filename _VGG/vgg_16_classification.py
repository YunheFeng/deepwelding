...
def main(args):
	...
	with tf.Session(graph=graph) as sess:
		...
		# write validation iamge names into files 
		...
		# when training and validation accuracies acheive both 1.00 for 5 continuous epochs, stop it
		num_epoch_completed_learning = 0
		...
		for epoch in range(args.num_epochs2):
			...
			if (train_acc == 1 and val_acc == 1):
				num_epoch_completed_learning += 1
			elif (num_epoch_completed_learning != 0):
				num_epoch_completed_learning = 0
			if (num_epoch_completed_learning == 5):
				# write accuracy results back to files
				with open(accuracy_file, 'a') as f:
					f.write('The training and validaion accuracies have acheived 1.00 for multiple continuous epochs. \n')
