...
def load_welding(batch_size, is_training=True):
	...
	trainX = np.float32(np.array(trainX))
	trainY = np.array(trainY)

	# calculate trX, trY, valX and valY
	num_tr = int(len(trainX)*(1 - validate_ratio))
	trX = trainX[:num_tr,:,:,:]
	trY = trainY[:num_tr]
	valX = trainX[num_tr:,:,:,:]
	valY = trainY[num_tr:]
	...
	for index,img_name in enumerate(test_filenames):
		...
		if (index == 0):
			...
			teX = np.concatenate((teX, img_reshape), axis=0)
	teX = np.float32(np.array(teX))
	
	# read testing labels, just convert the list to array
	teY = np.array(test_labels)
